...
__version__ = 'CN IDC Bulk show command automation for NXOS devices'
...
parser.add_argument('-i',help='Configuration Input File - Example: /home/user/rfc1234.txt - #### Be Sure to end your Input file with commmit and/or wr mem depending on device type #####',required=False)
parser.add_argument('-d',help='Devie type: ', type=str.upper, required=True)

args = parser.parse_args()
show_command_run = []
if args.i == None:
    show_command_run.append(raw_input('What command would you like to run?: \nExample: show version\n:'))
elif len(args.i) >= 0:
    show_command_run = open(args.i).read().splitlines()
else:
    parser.error('Enter valid Input')
    ...
    for a_dict in results:
        for identifier,v in a_dict.iteritems():
            ...
            if success:
                ...
                for i in out_string.keys():
                    print '\n\n'
                    print '*' * 80
                    print 'command -- {0}: \n {1}'.format(i,out_string[i])
                    print '*' * 80
                print '#' * 80
...
def worker_show_cmd(a_device, mp_queue):
    '''
    ...
    return_data[identifier] = (True, show_cmd)
    mp_queue.put(return_data)
...
def main():
    ...
    for a_device in devices:
        ...
        p = multiprocessing.Process(target=worker_show_cmd, args=(a_device, mp_queue))
        processes.append(p)
